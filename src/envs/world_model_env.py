...
class WorldModelEnv:
    ...
    def reset(self, **kwargs) -> ResetOutput:
        obs, act, _ = self.generator_init.send(self.num_envs)
        self.obs_buffer = obs
        self.act_buffer = act
        # Remove references to self.hx_rew_end and self.cx_rew_end
    ...
    def reset_dead(self, dead: torch.BoolTensor) -> None:
        obs, act, _ = self.generator_init.send(dead.sum().item())
    ...
    def step(self, act: torch.LongTensor) -> StepOutput:
        ...
        rew, end = torch.zeros(self.num_envs, dtype=torch.long, device=self.device), torch.zeros(self.num_envs, dtype=torch.long, device=self.device)
    ...
    def make_generator_init(
        ...
        while True:
            ...
            while c + num_dead <= len(obs_):
                ...
                c += num_dead
                num_dead = yield obs, act, (None, None)  # Return None instead of (hx, cx)
